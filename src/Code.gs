...
function doPost(e) {
  try {
    ...
    if (action === 'importQuestions') {
      var result = apiImportQuestionsCore_(params.rows);
    ...
    if (action === 'importRubrics') {
      var rubricResult = apiImportRubricsCore_(params.rubrics || params.rows || []);
      return ContentService.createTextOutput(JSON.stringify(rubricResult))
        .setMimeType(ContentService.MimeType.JSON);
    }
    if (action === 'importQuestionImages') {
      var imageResult = apiImportQuestionImagesCore_(params.images || params.rows || [], params.replaceExisting);
